...
@section('content')
    <div class="min-h-screen bg-slate-100 text-slate-800 antialiased" x-data="allowanceMatrix()">
        <div class="max-w-full mx-auto bg-white rounded-xl shadow-md border border-slate-200 overflow-hidden">
            ...
            <form id="matrix-form" action="{{ route('allowance-forms.store') }}" method="POST" class="p-6">
                @csrf
                ...
                <div
                    ...
                    <div>
                        ...
                        <select name="program"
                            ...
                            <option value="RITI" {{ old('program') == 'RITI' ? 'selected' : '' }}>
                                RITI
                            </option>

                            <option value="SACHAS" {{ old('program') == 'SACHAS' ? 'selected' : '' }}>
                                SACHAS
                            </option>

                            <option value="MACOR" {{ old('program') == 'MACOR' ? 'selected' : '' }}>
                                MACOR
                            </option>

                            <option value="PALI" {{ old('program') == 'PALI' ? 'selected' : '' }}>
                                PALI
                            </option>
                        </select>
                    ...
                    <div>
                        ...
                        <input type="text" name="donor" value="{{ old('donor') }}"
                            class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 transition focus:border-green-700 focus:ring-1 focus:ring-green-700 text-slate-800 shadow-xs"
                    ...
                    <div>
                        <label class="block font-bold text-slate-600 mb-2 tracking-wide uppercase text-sm">Donor
                            Code</label>
                        <input type="text" name="donor_code" value="{{ old('donor_code') }}"
                            class="w-full bg-white border border-slate-300 rounded-lg px-3 py-2 transition focus:border-green-700 focus:ring-1 focus:ring-green-700 text-slate-800 shadow-xs"
                                ...
                                @foreach ($donorLogos as $donor)
                                    <label
                                        ...
                                        <input type="checkbox" name="donor_logo_ids[]" value="{{ $donor->id }}"
                                            ...
                                            @checked(in_array($donor->id, old('donor_logo_ids', $selectedDonors ?? [])))>
                    ...
                    <table class="w-full text-xs text-left border-collapse min-w-[1250px]">
                        <thead>
                            <tr
                                ...
                                <th class="p-2.5 border-r border-slate-300 text-center w-[20%]">Name <br> (Position &
                                    organization)</th>
                                <th class="p-2.5 border-r border-slate-300 w-[6%] text-center">Sex</th>
                                <th class="p-2.5 border-r border-slate-300 text-center w-[30%]">Village, Commune, District,
                                    Province <br> (be specific)</th>
                                <th class="p-2.5 border-r border-slate-300 w-[15%] bg-slate-50 text-slate-500">Allowance
                                    Type</th>
                                ...
                                <th class="p-2.5 border-r border-slate-300 w-[8%] text-right bg-slate-100/80">Total</th>
                                <th class="p-2.5 w-[10%] text-center bg-slate-200/50">Actions</th>
                            </tr>
                        </thead>
                                                ...
                                                <div class="flex divide-x divide-slate-200">
                                                    ...
                                                    <template x-for="(day, dIdx) in dates" :key="dIdx">
                                                        ...
                                                        <input type="number" step="0.01"
                                                            ...
                                                            class="w-full bg-transparent border-0 text-center text-xs px-1 py-1.5 font-medium transition focus:ring-1 focus:ring-green-700 focus:bg-green-50/40"
                                                            placeholder="-">
